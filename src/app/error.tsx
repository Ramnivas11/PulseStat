...
export default function Error({
  ...
  useEffect(() => {
    // Log to browser console — never use server logger in client components
    console.error("[PulseStat] Page error:", error.message, error.digest);
  }, [error]);
    ...
    <div className="flex h-[50vh] items-center justify-center p-6 animate-in fade-in zoom-in duration-300">
      <Card className="max-w-md w-full border-red-200 bg-red-50/50 dark:border-red-900/50 dark:bg-red-950/20">
        <CardContent className="flex flex-col items-center p-8 text-center space-y-4">
          ...
          <button
            onClick={() => reset()}
            className="mt-2 inline-flex h-10 items-center justify-center rounded-md bg-red-600 px-6 text-sm font-medium text-white transition-colors hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
          >
            Try again
          </button>
        </CardContent>
