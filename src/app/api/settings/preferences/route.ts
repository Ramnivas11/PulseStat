...
export async function GET() {
  try {
    ...
    console.error("/api/settings/preferences GET error:", err instanceof Error ? err.message : String(err));
...
export async function PATCH(req: Request) {
  try {
    ...
    const merged = { ...((user?.settings as Record<string, unknown>) || {}), ...(body || {}) };
    ...
    console.error("/api/settings/preferences PATCH error:", err instanceof Error ? err.message : String(err));
    return safeJsonResponse({ error: "Internal server error" }, 500);
  }
}
