...
import { Check, Copy, ExternalLink, Trash2, Loader2, BarChart3 } from "lucide-react";
...
export function WebsiteCard({ id, name, domain, siteKey }: WebsiteCardProps) {
  ...
  const snippet = `<script async src="${scriptUrl}" data-site-id="${siteKey}" data-api-url="${siteConfig.url}"></script>`;
      ...
      {id && (
        <div className="flex items-center gap-2 border-t bg-muted/20 px-6 py-3">
          ...
          <Button size="sm" variant="outline" asChild>
            <Link href={`/websites/${id}/settings`}>Settings</Link>
          </Button>
        </div>
      )}
    </Card>
