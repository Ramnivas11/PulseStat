...
function createRateLimiter(
  ...
  if (!redis) {
    if (process.env.NODE_ENV === "production") {
      logWarn(
        "Upstash Redis is not configured; using per-instance in-memory rate limits",
        { prefix }
      );
    }
    const parts = window.split(" ");
    const amount = Number.parseInt(parts[0] ?? "1", 10);
    const unit = (parts[1] ?? "m") as "s" | "m" | "h";
    const multiplier =
      unit === "h" ? 3_600_000 : unit === "m" ? 60_000 : 1_000;
...
export const trackRateLimit = createRateLimiter(
  120,
  "1 m",
  "@pulsestat/ratelimit/track"
);
export const authRateLimit = createRateLimiter(
  10,
  "1 m",
  "@pulsestat/ratelimit/auth"
);
